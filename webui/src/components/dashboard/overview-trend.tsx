...
// OverviewTrend renders a time-series area chart of the pool's usage.
//
// Three orthogonal knobs the operator can tweak from the toolbar:
//   1. metric  — charged_credits (money) / total_credits (incl. free) / requests
//   2. split   — total / by model_alias / by api_key_id / by group_id
//   3. window  — provided by the parent via props (24h/7d/15d/30d/custom)
//
// The chart always queries /admin/usage/aggregate with group_by = [<bucket>,
// <split>], where <bucket> is billing_hour for hour-granularity windows and
// billing_day for day-granularity windows. When split=total we drop the
// second dim so the server returns one row per time bucket.
//
// Series identity for the ChartConfig is derived from the raw
// api_key_id / group_id / model_alias values in the response. For key/group
// splits we side-load /admin/keys and /admin/groups to substitute the
// human-readable name in the tooltip/legend without needing a join on the
// backend.

type Metric = "charged" | "total" | "requests";
type Split = "total" | "model_alias" | "api_key_id" | "group_id";

// Options carry i18n keys rather than rendered strings so language flips
// re-render without needing to reconstruct the arrays.
const METRIC_OPTIONS: { value: Metric; labelKey: string }[] = [
  { value: "charged", labelKey: "dashboard.trend.metric.charged" },
  { value: "total", labelKey: "dashboard.trend.metric.total" },
  { value: "requests", labelKey: "dashboard.trend.metric.requests" },
];

const SPLIT_OPTIONS: { value: Split; labelKey: string }[] = [
  { value: "total", labelKey: "dashboard.trend.split.total" },
  { value: "model_alias", labelKey: "dashboard.trend.split.model" },
  { value: "api_key_id", labelKey: "dashboard.trend.split.key" },
  { value: "group_id", labelKey: "dashboard.trend.split.group" },
];
...
export function OverviewTrend({ window }: Props) {
          ...
          {t("dashboard.trend.subtitle", {
            ...
            granularity: t(
              window.granularity === "hour"
                ? "dashboard.trend.granularity.hour"
                : "dashboard.trend.granularity.day",
            ),
            split: t(
              SPLIT_OPTIONS.find((o) => o.value === split)?.labelKey ?? "",
            ),
          })}
        </CardDescription>
      ...
      <CardContent>
        {usageQ.isLoading ? (
          ...
          <p className="text-sm text-destructive">
            {usageQ.error instanceof Error ? usageQ.error.message : "Failed to load"}
          </p>
        ) : rows.length === 0 ? (
            ...
            <AreaChart data={rows}>
              ...
              <XAxis
                ...
                tickFormatter={(v: string) =>
                  format(new Date(v), window.granularity === "hour" ? "MM/dd HH:mm" : "MM/dd")
                }
              />
              <YAxis
                tickLine={false}
...
// buildSeries transposes the aggregate rows (long form, one per bucket ×
// dim) into wide form suitable for Recharts (one row per bucket, one
// column per series). Series are ordered by total descending so the
// stacking looks stable and the legend surfaces the heavy hitters first.
function buildSeries(
  ...
  const valueOf = (r: UsageAggRow) => {
    switch (metric) {
      ...
      case "total":
        return r.total_credits_h / 100;
      case "requests":
        return r.request_count;
    }
  };

  // Collect buckets and per-series totals.
  ...
  // Cap total series to 5 (chart palette size) so the chart never looks
  // like spaghetti — extras roll into "Other".
  ...
  const series: Series[] = primary.map((k) => ({ key: k, label: seriesLabel[k]! }));
  ...
  const rows = orderedBuckets.map((b) => {
    ...
    if (overflow.length > 0) {
      row["__other"] = overflow.reduce((sum, k) => sum + (cell[b]?.[k] ?? 0), 0);
    }
    return row;
  });
